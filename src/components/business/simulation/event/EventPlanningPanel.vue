...
// 向父组件传递事�?const emit = defineEmits(['event-saved']);
...
const eventTemplates = ref<EventTemplate[]>([
  {
    ...
    name: '周末狂欢',
    icon: '🎉',
    description: '适合周末的简单活动，提升用户活跃�?,
    config: {
      name: '周末狂欢活动',
      type: 'limited',
      rewards: [{ type: 'experience', name: '双倍经�?, quantity: 2 }],
      duration: 2,
      target: 'all',
      conditions: {
        level: false,
  ...
  },
  {
    ...
    name: '社区争霸',
    icon: '🏆',
    description: '社区竞赛活动，提升用户粘�?,
    config: {
      name: '社区争霸�?,
      type: 'community',
      rewards: [{ type: 'title', name: '社区王�?, quantity: 1 }],
      duration: 10,
      target: 'active',
      conditions: {
        level: false,
...
// 选中的活动类�?const selectedEventType = ref('limited');
...
const impactPreview = ref<ImpactPreview[]>([
  { metric: '用户活跃�?, change: 15, description: '预计提升15%的日活跃用户' },
  { metric: '玩家满意�?, change: 8, description: '预计提升8%的玩家满意度' },
  { metric: '收入', change: 20, description: '预计提升20%的活动期间收�? },
  { metric: '留存�?, change: 5, description: '预计提升5%�?日留存率' },
]);
...
// 快速策�?const quickPlan = (): void => {
  // 随机选择一个模�?  const randomTemplate =
...
const generateImpactPreview = (eventType: string): void => {
  // 根据活动类型生成不同的影响预�?  if (eventType === 'limited') {
    impactPreview.value = [
      {
        metric: '用户活跃�?,
        change: 15,
      ...
      { metric: '收入', change: 10, description: '预计提升10%的活动期间收�? },
      { metric: '留存�?, change: 5, description: '预计提升5%�?日留存率' },
    ];
    ...
    impactPreview.value = [
      ...
      { metric: '收入', change: 30, description: '预计提升30%的活动期间收�? },
      { metric: '留存�?, change: 8, description: '预计提升8%�?日留存率' },
    ];
    ...
    impactPreview.value = [
      ...
      { metric: '收入', change: 15, description: '预计提升15%的活动期间收�? },
      { metric: '留存�?, change: 10, description: '预计提升10%�?日留存率' },
    ];
  }
};
...
.quick-plan-btn:hover {
  ...
  transform: translateY(-2px);
  box-shadow: 0 4px 16px rgb(251 191 36 / 50%);
}
