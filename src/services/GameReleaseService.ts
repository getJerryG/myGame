...
export class GameReleaseService {
  ...
  // 获取待发布的内部列表
  async getPendingReleases(): Promise<GameReleaseData[]> {
  ...
  // 执行游戏发布操作
  async releaseGame(_releaseData: GameReleaseData): Promise<GameReleaseResult> {
    // 这里可以添加实际的发布逻辑
    // 模拟发布操作
  ...
  // 获取版本历史
  async getVersionHistory(_gameId: string): Promise<any[]> {
  ...
  // 检查游戏发布类型是否有效
  validateReleaseType(type: string): boolean {
    const validTypes = this.getInternalReleaseTypes().map((t) => t.id);
    return validTypes.includes(type);
  }

  // 生成发布日志
  generateReleaseNotes(releaseData: GameReleaseData): string {
