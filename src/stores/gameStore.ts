...
export const useGameStore = defineStore('game', {
  ...
  getters: {
    ...
    expProgressPercent: (state) => {
      return Math.floor((state.plannerExp / getMaxExp(state.plannerLevel, state.plannerSubLevel, state.levelInRank)) * 100);
    },
  ...
  actions: {
    ...
    calculateIncome() {
      ...
      const dailyIncome = Math.floor(this.popularity * this.wordOfMouth / 100 + this.heroCount * 100 + this.skinCount * 50);
      ...
      this.ongoingProjects.forEach(project => {
        ...
        if (project.progress >= 100) {
          ...
          if (project.type === 'hero') {
            ...
            this.onlineSkins.push({ id: project.id, name: project.name, heroName: project.heroName, ...project.details });
            this.skinCount++;
          }
        }
      });
    ...
    checkPromotion() {
      ...
      if (this.canPromote) {
        ...
        // 获取下一个等级信息
        const { nextLevel, nextSubLevel, nextLevelInRank } = getNextLevelInfo(this.plannerLevel, this.plannerSubLevel, this.levelInRank);
    ...
    calculateSkinSales() {
      ...
      this.onlineSkins.forEach(skin => {
        ...
        const baseSales = skin.quality === 'legendary' ? 1000 : 
                         skin.quality === 'epic' ? 500 : 
                         skin.quality === 'rare' ? 200 : 50;
        
        // 品质系数（根据皮肤品质设置不同的系数）
        const qualityCoeff = skin.quality === 'legendary' ? 2.0 : 
                             skin.quality === 'epic' ? 1.5 : 
                             skin.quality === 'rare' ? 1.2 : 1.0;
        
        // 热度系数（根据游戏热度计算）
        const popularityCoeff = Math.max(0.1, this.popularity / 100);
        
        // 职级加成（根据策划职级设置不同的加成）
        const levelCoeff = this.plannerLevel === '总监' ? 1.5 : 
                           this.plannerLevel === '经理' ? 1.3 : 
                           this.plannerLevel === '专家' ? 1.2 : 
                           this.plannerLevel === '资深' ? 1.1 : 1.0;
        ...
        // 将皮肤销售收入计入总流水和当前资金
        const skinPrice = skin.quality === 'legendary' ? 1688 : 
                          skin.quality === 'epic' ? 888 : 
                          skin.quality === 'rare' ? 288 : 188;
...
// 辅助函数：获取下一个等级信息
function getNextLevelInfo(level: string, subLevel: string, levelInRank: number): { nextLevel: string; nextSubLevel: string; nextLevelInRank: number } {
  let nextLevel = level;
  ...
  if (nextLevelInRank > 2) {
    ...
    if (subLevel === 'I' && levelInRank === 2) {
      ...
      nextLevel = levelIndex < plannerLevels.length - 1 ? plannerLevels[levelIndex + 1] : plannerLevels[plannerLevels.length - 1];
    }
  }
