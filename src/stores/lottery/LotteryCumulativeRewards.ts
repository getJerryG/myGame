...
// 创建并导出累计奖励store
export const useLotteryCumulativeRewardsStore = defineStore('lotteryCumulativeRewards', {
  state: (): CumulativeRewardsState => ({
    ...
    rewardConfigs: {
      currency: [
        { type: 'currency', minAmount: 100, maxAmount: 500, probability: 0.5 },
        { type: 'currency', minAmount: 500, maxAmount: 1000, probability: 0.3 },
        { type: 'currency', minAmount: 1000, maxAmount: 2000, probability: 0.15 },
        { type: 'currency', minAmount: 2000, maxAmount: 5000, probability: 0.05 },
      ],
      item: [
        { type: 'item', minAmount: 1, maxAmount: 3, probability: 0.4, itemId: 'common-item', itemCount: 1 },
        { type: 'item', minAmount: 1, maxAmount: 2, probability: 0.25, itemId: 'rare-item', itemCount: 1 },
        { type: 'item', minAmount: 1, maxAmount: 1, probability: 0.15, itemId: 'epic-item', itemCount: 1 },
        { type: 'item', minAmount: 1, maxAmount: 1, probability: 0.05, itemId: 'legendary-item', itemCount: 1 },
      ],
      special: [
        { type: 'special', minAmount: 1, maxAmount: 1, probability: 0.04, specialEffect: 'double-currency' },
        { type: 'special', minAmount: 1, maxAmount: 1, probability: 0.03, specialEffect: 'free-draw' },
        { type: 'special', minAmount: 1, maxAmount: 1, probability: 0.02, specialEffect: 'guaranteed-epic' },
        { type: 'special', minAmount: 1, maxAmount: 1, probability: 0.01, specialEffect: 'legendary-chance' },
      ],
    },
    // 当前可领取的累计奖励
    availableRewards: [],
  ...
  getters: {
    ...
    // 获取当前可领取的累计奖励
    getAvailableRewards: (state) => state.availableRewards.filter((reward) => !reward.claimed),
  ...
  actions: {
    ...
    selectRewardType(): CumulativeRewardType | null {
      ...
      const totalProbability = {
        currency: this.rewardConfigs.currency.reduce((sum, config) => sum + config.probability, 0),
        item: this.rewardConfigs.item.reduce((sum, config) => sum + config.probability, 0),
        special: this.rewardConfigs.special.reduce((sum, config) => sum + config.probability, 0),
      };

      const total = totalProbability.currency + totalProbability.item + totalProbability.special;
      const random = Math.random() * total;

      let cumulative = 0;
      if (random < cumulative + totalProbability.currency) {
        return 'currency';
      } else if (random < (cumulative += totalProbability.currency) + totalProbability.item) {
        return 'item';
      } else if (random < (cumulative += totalProbability.item) + totalProbability.special) {
        return 'special';
      }
    ...
    // 根据奖励类型生成奖励
    generateRewardByType(type: CumulativeRewardType): CumulativeReward | null {
      const configs = this.rewardConfigs[type];
      if (!configs || configs.length === 0) return null;

      // 计算总概率
      const totalProbability = configs.reduce((sum, config) => sum + config.probability, 0);
      ...
      const amount = Math.floor(
        Math.random() * (selectedConfig.maxAmount - selectedConfig.minAmount + 1) + selectedConfig.minAmount
      );
    ...
    claimCumulativeReward(rewardId: string): boolean {
      ...
      // 查找奖励
      const rewardIndex = this.availableRewards.findIndex((reward) => reward.id === rewardId && !reward.claimed);
      if (rewardIndex === -1) {
        return false;
      }
