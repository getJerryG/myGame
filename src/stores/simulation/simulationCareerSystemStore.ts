...
// 创建并导出职业系统store
export const useSimulationCareerSystemStore = defineStore('simulationCareerSystem', {
  ...
  getters: {
    ...
    getCurrentSubLevel: (state) => {
      return state.careerLevels[state.levelIndex].subLevels[state.subLevelIndex];
    },
  ...
  actions: {
    ...
    addExp(amount: number): CareerPromotionResult {
      ...
      while (this.exp >= this.currentLevelExpRequired) {
        ...
        // 检查是否可以升级到下一个小等级
        if (this.subLevelIndex < this.careerLevels[this.levelIndex].subLevels.length - 1) {
          // 升级到下一个小等级
          this.subLevelIndex++;
          this.currentLevelExpRequired = this.careerLevels[this.levelIndex].subLevels[this.subLevelIndex].expRequired;
    ...
    resetCareerSystem() {
      ...
      this.subLevelIndex = 0;
      this.exp = 0;
      this.currentLevelExpRequired = this.careerLevels[0].subLevels[0].expRequired;
    },
