...
// 创建并导出合作store
export const useSimulationCollaborationStore = defineStore('simulationCollaboration', {
  ...
  getters: {
    ...
    activeCollaborations: (state) => {
      return state.currentCollaborations.filter((collab) => collab.status === 'active');
    },

    // 获取谈判中的合作项目
    negotiatingCollaborations: (state) => {
      return state.currentCollaborations.filter((collab) => collab.status === 'negotiating');
    },

    // 获取待处理的合作请求
    pendingRequests: (state) => {
      return state.collaborationRequests.filter((request) => request.status === 'pending');
    },

    // 根据ID获取合作项目
    getCollaborationById: (state) => (collabId: string) => {
      return [...state.currentCollaborations, ...state.historicalCollaborations].find(
  ...
  actions: {
    // 创建合作项目
    createCollaboration(collabData: Omit<Collaboration, 'id' | 'progress' | 'currentPhase'>) {
      if (this.activeCollaborations.length >= this.collaborationConfig.maxActiveCollaborations) {
    ...
    // 更新合作状态
    updateCollaborationStatus(collabId: string, status: 'negotiating' | 'active' | 'completed' | 'canceled') {
      ...
      if (collaboration) {
        ...
        if (status === 'completed' || status === 'canceled') {
          const currentIndex = this.currentCollaborations.findIndex((c) => c.id === collabId);
          if (currentIndex !== -1) {
            const [completedCollab] = this.currentCollaborations.splice(currentIndex, 1);
    ...
    // 更新合作进度
    updateCollaborationProgress(collabId: string, progress: number, phase?: string) {
      const collaboration = this.getCollaborationById(collabId);
      if (collaboration) {
        ...
        // 如果进度达到100%，自动标记为完成
        if (collaboration.progress >= 100 && collaboration.status === 'active') {
