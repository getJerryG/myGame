...
interface CoreValue {
  ...
  name: string;
  description: string;
  category: 'innovation' | 'quality' | 'user-centric' | 'teamwork' | 'integrity';
...
export const useSimulationCoreValuesStore = defineStore('simulationCoreValues', {
    ...
    coreValues: [
      {
        id: 'value_innovation',
        name: '创新',
        description: '不断探索新的游戏机制和设计理念，保持产品的竞争力和吸引力',
        category: 'innovation',
        influenceScore: 85,
  ...
  getters: {
    ...
    valuesScore: (state) => {
      ...
      const totalScore = state.coreValues.reduce((sum, value) => sum + value.influenceScore, 0);
    ...
    implementationScore: (state) => {
      if (state.coreValues.length === 0) return 0;
      const totalScore = state.coreValues.reduce((sum, value) => sum + value.implementationLevel, 0);
      return Math.round(totalScore / state.coreValues.length);
    },

    // 获取需要审查的价值观
    valuesNeedingReview: (state) => {
      const reviewThreshold = Date.now() - state.valuesConfig.reviewPeriodDays * 24 * 60 * 60 * 1000;
      return state.coreValues.filter((value) => value.lastReviewed < reviewThreshold);
    },
  ...
  actions: {
    ...
    // 审查价值观
    reviewValue(valueId: string, updates?: Partial<Omit<CoreValue, 'id' | 'lastReviewed' | 'status'>>) {
      const value = this.coreValues.find((v) => v.id === valueId);
      if (value) {
        if (updates) {
