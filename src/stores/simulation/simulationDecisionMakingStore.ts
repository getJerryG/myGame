  ...
  'simulationDecisionMaking',
  {
    ...
    getters: {
      ...
      getDecisionById: (state) => (decisionId: string) => {
        return [...state.currentDecisions, ...state.historicalDecisions].find(
    ...
    actions: {
      ...
      updateImplementationProgress(decisionId: string, progress: number) {
        const decision = this.getDecisionById(decisionId);
        if (decision) {
          decision.implementationProgress = Math.max(
            0,
          ...
          if (
            ...
            if (index !== -1) {
              const [implementedDecision] = this.currentDecisions.splice(
                index,
                1,
              );
      ...
      getDecisionImpact(decisionId: string) {
        return this.decisionImpacts.find(
          (impact) => impact.decisionId === decisionId,
        );
      },
    },
