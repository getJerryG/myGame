...
// 创建并导出英雄发展store
export const useSimulationHeroDevelopmentStore = defineStore(
  "simulationHeroDevelopment",
  {
    ...
    actions: {
      ...
      createHero(
        ...
        const newHero: Hero = {
          ...
          experience: 0,
          experienceToNextLevel:
            this.developmentConfig.baseExperienceToNextLevel,
          ...
          while (hero.experience >= hero.experienceToNextLevel) {
            ...
            Object.keys(hero.attributes).forEach((attr) => {
              ...
              hero.attributes[attribute] +=
                this.developmentConfig.attributeGrowthRate;
            });
          }
        }
      },
          ...
          if (skill && skill.level < this.developmentConfig.maxSkillLevel) {
            ...
            if (skillConfig) {
              const skillExpToNextLevel = Math.round(
                skillConfig.baseExperienceToNextLevel *
                  Math.pow(1.2, skill.level - 1),
              );
      ...
      trainHero(heroId: string, days = 1) {
        ...
        if (hero) {
          ...
          // 随机提升一个技能
          const randomSkill =
            hero.skills[Math.floor(Math.random() * hero.skills.length)];
          this.addSkillExperience(
            heroId,
            randomSkill.id,
            trainingExperience * 0.5,
          );
