...
// 创建并导出新手目标store
export const useSimulationNewbieGoalsStore = defineStore("simulationNewbieGoals", {
  ...
  getters: {
    ...
    activeNewbieGoals: (state) => {
      return state.newbieGoals.filter((goal) => goal.status === "active").sort((a, b) => a.order - b.order);
    },

    // 获取即将完成的新手目标
    nearCompletionGoals: (state) => {
      return state.newbieGoals.filter((goal) => goal.status === "active" && goal.progress >= 80);
    },
    ...
    getGoalsByType: (state) => (type: NewbieGoal["type"]) => {
      return [...state.newbieGoals, ...state.completedNewbieGoals].filter((goal) => goal.type === type);
    },
  },
  ...
  actions: {
    ...
    updateGoalProgress(goalId: string, currentValue: number) {
      ...
      if (goal?.status === "active") {
        ...
        goal.progress = Math.min(100, Math.round((currentValue / goal.target) * 100));
    ...
    // 更新进度统计
    updateProgressStats() {
      const totalGoals = this.newbieGoals.length + this.completedNewbieGoals.length;
    ...
    // 添加新的新手目标
    addNewbieGoal(goalData: Omit<NewbieGoal, "id" | "progress" | "status" | "completedAt">) {
