...
interface OperationPermission {
  ...
  name: string;
  description: string;
  type: 'game_operation' | 'user_management' | 'financial' | 'content_management';
...
// 创建并导出操作权限store
export const useSimulationOperationPermissionsStore = defineStore('simulationOperationPermissions', {
  ...
  getters: {
    ...
    grantedPermissions: (state) => {
      return state.operationPermissions.filter((permission) => permission.isGranted);
    },

    // 根据类型获取权限
    getPermissionsByType: (state) => (type: OperationPermission['type']) => {
      return state.operationPermissions.filter((permission) => permission.type === type);
    },

    // 获取可授予的权限
    grantablePermissions: (state) => (userLevel: number) => {
      return state.operationPermissions.filter(
        (permission) => permission.requiredLevel <= userLevel && !permission.isGranted
      );
    },

    // 根据ID获取权限
    getPermissionById: (state) => (permissionId: string) => {
      return state.operationPermissions.find((permission) => permission.id === permissionId);
    },
  },
  ...
  actions: {
    ...
    // 添加新权限
    addPermission(permissionData: Omit<OperationPermission, 'id' | 'usageCount' | 'lastUsed'>) {
