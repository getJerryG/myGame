...
// 创建并导出皮肤开发系统store
export const useSimulationSkinDevelopmentStore = defineStore('simulationSkinDevelopment', {
  ...
  getters: {
    ...
    getDevelopingSkins: (state) => {
      return state.skins.filter((skin) => skin.developmentStatus === 'development');
    },

    // 获取所有已发布的皮肤
    getReleasedSkins: (state) => {
      return state.skins.filter((skin) => skin.developmentStatus === 'released');
    },
    ...
    getCurrentDevelopingSkin: (state) => {
      ...
      return state.skins.find((skin) => skin.id === state.currentDevelopingSkinId);
    },

    // 获取已发布皮肤的数量
    getReleasedSkinsCount: (state) => {
      return state.skins.filter((skin) => skin.developmentStatus === 'released').length;
    },
  ...
  actions: {
    // 计算皮肤开发成本
    calculateDevelopmentCost(quality: Skin['quality'], isDual: boolean, isLimited: boolean): number {
      let baseCost = 0;
    ...
    // 计算皮肤开发时间
    calculateDevelopmentTime(quality: Skin['quality'], isDual: boolean): number {
      let baseTime = 0;
    ...
    createSkinProject(
      ...
      // 检查资金是否足够
      const developmentCost = this.calculateDevelopmentCost(quality, isDual, isLimited);
    ...
    createDevelopmentTasks(skinId: string): void {
      // 清除旧任务
      this.developmentTasks = this.developmentTasks.filter((task) => task.skinId !== skinId);
    ...
    updateDevelopmentProgress() {
      ...
      // 计算每日进度
      const dailyProgress = this.availableResources.dailyProductivity * this.availableResources.teamSize;

      // 更新未完成任务的进度
      const ongoingTasks = this.developmentTasks.filter(
        (task) => task.skinId === this.currentDevelopingSkinId && !task.completed
      );

      if (ongoingTasks.length === 0) {
        // 所有任务已完成，标记皮肤开发完成
        const skin = this.skins.find((s) => s.id === this.currentDevelopingSkinId);
        if (skin) {
      ...
      const currentTotalProgress = this.developmentTasks
        .filter((task) => task.skinId === this.currentDevelopingSkinId)
        .reduce((total, task) => total + (task.progress / 100) * task.requiredTime, 0);

      this.totalDevelopmentProgress = Math.floor((currentTotalProgress / totalRequiredProgress) * 100);
    },
    ...
    updateReleasedSkinStats() {
      this.skins
        ...
        .forEach((skin) => {
          // 模拟每日销售数据更新
          const dailySales = Math.floor((Math.random() * 100 * skin.popularity) / 100);
          ...
          // 更新流行度（基于销售和评价）
          skin.popularity = Math.min(100, skin.popularity + (dailySales > 50 ? 1 : 0));
        });
      ...
      this.statistics.averageSales =
        this.skins.filter((skin) => skin.developmentStatus === 'released').length > 0
          ? Math.floor(
              this.statistics.totalRevenue / this.skins.filter((skin) => skin.developmentStatus === 'released').length
            )
          : 0;
    },
