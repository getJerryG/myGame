...
// 创建并导出任务系统store
export const useSimulationTaskSystemStore = defineStore("simulationTaskSystem", {
  ...
  getters: {
    ...
    // 获取成长任务
    getGrowthTasks: (state) => state.tasks.filter((task) => task.type === "growth"),

    // 获取日常任务
    getDailyTasks: (state) => state.tasks.filter((task) => task.type === "daily"),

    // 获取随机任务
    getRandomTasks: (state) => state.tasks.filter((task) => task.type === "random"),

    // 获取特殊任务
    getSpecialTasks: (state) => state.tasks.filter((task) => task.type === "special"),

    // 获取已完成待领取奖励的任务
    getCompletedTasksWithReward: (state) => state.tasks.filter((task) => task.status === "completed"),

    // 获取已领取奖励的任务
    getRewardedTasks: (state) => state.tasks.filter((task) => task.status === "rewarded"),
  ...
  actions: {
    ...
    generateGrowthTasks(_level = 1) {
      // 根据游戏等级生成成长任务
      // 避免生成重复任务
      const existingGrowthTasks = this.tasks.filter((task) => task.type === "growth" && task.status === "pending");
      ...
      growthTaskTemplates.forEach((template) => {
        ...
        const hasSameTypeTask = this.tasks.some(
          (task) => task.type === "growth" && task.status === "pending" && task.name === template.name
        );
    ...
    generateDailyTasks(force = false) {
      ...
      if (!force && timeSinceLastDailyRefresh < this.taskTypeConfig.daily.refreshInterval) {
        return;
      }
    ...
    generateRandomTasks() {
      ...
      if (timeSinceLastRandomRefresh < this.taskTypeConfig.random.refreshInterval) {
        return;
      }
      ...
      // 随机选择一个随机任务模板生成任务
      const randomTemplate = randomTaskTemplates[Math.floor(Math.random() * randomTaskTemplates.length)];
    ...
    handleUserActivity(eventType: string, eventData: any) {
      ...
      this.tasks.forEach((task) => {
        ...
        task.triggers.forEach((trigger) => {
          ...
          } else if (trigger.type === "action" && trigger.eventType === eventType) {
            // 动作触发的任务
            this.updateTaskProgress(task.id, task.progress + 1);
          }
        });
    ...
    cleanupExpiredTasks(type?: TaskType) {
      ...
      this.tasks = this.tasks.filter((task) => {
        // 检查任务是否过期
        const isExpired = task.expiredAt && now > task.expiredAt && task.status === "pending";
    ...
    claimTaskReward(taskId: string): TaskReward[] | null {
      ...
      if (task.completedAt && task.createdAt) {
        ...
        this.statistics.averageCompletionTime =
          (this.statistics.averageCompletionTime * (this.statistics.totalTasksCompleted - 1) + completionTime) /
          this.statistics.totalTasksCompleted;
      }
