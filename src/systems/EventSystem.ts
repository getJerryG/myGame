...
export class EventSystem {
  ...
  private generateEventEffects(
    ...
    switch (type) {
      ...
      case "system":
        ...
        break;
      case "random": {
        const randomEffectTypes: Array<"money" | "reputation" | "popularity"> =
          ["money", "reputation", "popularity"];
        effects.push({
          type: randomEffectTypes[
            Math.floor(Math.random() * randomEffectTypes.length)
          ],
  ...
  private checkRequirements(
    ...
    for (const requirement of requirements) {
      const currentValue = this.getCurrentValue(requirement.type);
      if (
  ...
  // 获取当前值
  private getCurrentValue(type: SimulationEventRequirement["type"]): number {
    if (!this.state) return 0;

    switch (type) {
      case "money":
        return this.state.stats.currentFunds;
      case "reputation":
        // 假设reputation在stats中有记录
        return this.state.stats.currentLevel * 10;
      case "popularity":
        // 假设popularity在stats中有记录
        return this.state.stats.currentLevel * 5;
      case "level":
        return this.state.stats.currentLevel;
      case "heroCount":
        // 假设heroCount在stats中有记录
        return this.state.stats.totalEvents;
      case "skinCount":
        // 假设skinCount在stats中有记录
        return this.state.stats.totalDecisions;
      default:
        return 0;
    }
  }
  ...
  private checkRequirement(
    currentValue: number,
    operator: SimulationEventRequirement["operator"],
